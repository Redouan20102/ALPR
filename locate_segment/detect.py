...
flags.DEFINE_list('images', './test', 'path to input image')  # its modified by yasser
...
def main(_argv):
    ...
    for count, image_path in enumerate(images, 1):  # ctd commence de 1 jusqua à lent(images).
        ...
        if FLAGS.crop:
            ...
            crop_path = os.path.join(os.getcwd(), 'detections/plate', image_name)
        ...
        nj = randint(100, 999) / 100.00
        cv2.imwrite('./detections/localisation/' + str(image_name) + '.jpg', image)

        # cv2.imwrite(FLAGS.output +  str(image_name) + '.png', image)

        # cv2.imwrite(FLAGS.output + 'image_name' + image_name + '.png', image)
