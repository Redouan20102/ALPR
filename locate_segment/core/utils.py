...
def recognize_plate(img, coords):
    ...
    for cnt in sorted_contours:
        ...
        if w / h < 0.8 and (h * w > 1000) and abs(x - Xs[index]) > 15:
            initial = chr(ord(initial) + 1)
            index = index + 1
            Xs.append(x)
            cropped_img = img[int(y) - 5:int(y + h) + 5, int(x) - 5:int(x + w) + 5]
            W = 100
            ...
            if width != 0 and height != 0:
                imgScale1 = W / width
                imgScale2 = W / height
                newX, newY = cropped_img.shape[1] * imgScale1, cropped_img.shape[0] * imgScale2
                cropped_img = cv2.resize(cropped_img, (int(newX), int(newY)))
                # cv2.imwrite('./detections/seg/' +  str(index) + '.png', cropped_img)

            # cv2.imshow("red",cropped_img)
            # cv2.waitKey(0)
            # print(cropped_img.shape)
            if cropped_img.shape == (100, 100):

                a = 0
                for haut in range(100):
                    if cropped_img[0, haut] > 250:
                        a = a + 1
                    if cropped_img[haut, 0] > 250:
                        a = a + 1
                    if cropped_img[99, haut] > 250:
                        a = a + 1
                    if cropped_img[haut, 99] > 250:
                        a = a + 1
                        # cropped_img =  (cropped_img * 1.0 )/255
        try:
            cv2.imwrite('./detections/seg/' + str(initial) + '.png', cropped_img)
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                try:
                    ...
                    print('failed due to empty image 2')
                    plate_number = ""
